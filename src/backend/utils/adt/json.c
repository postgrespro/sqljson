...
static bool
jsontFillValue(JsonIterator **pit, JsonbValue *res, bool skipNested,
		...
		case jbvDatetime:
			{
				...
				str = DatumGetCString(
					DirectFunctionCall1(typoutput, jbv->val.datetime.value));
...
JsonbValueToJson(JsonbValue *jbv)
{
	...
	{
		*(JsonContainerData *) &json->root = *(JsonContainer *) jbv->val.binary.data;
		return json;
	}
...
int
compareJsonContainers(JsonContainer *a, JsonContainer *b)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("json comparison is not implemented")));
	return 0;
}
