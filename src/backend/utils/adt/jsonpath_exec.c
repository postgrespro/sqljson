					...
					JsonPathCastCache *cache, JsonbValue *jbv, bool *isnull)
{
	...
	*isnull = jbv->type == jbvNull; /* FIXME coercion to json/jsonb */
	...
	if (cache->returning.typid == JSONXOID)
		return *isnull ? (Datum) 0 : JsonbGetDatum(JsonbValueToJsonb(jbv));
				 ...
				 Datum value, bool isnull, bool isbool, JsonValueList *found)
{
	...
	else
	{
		...
		if (isnull)
			...
			JsonbInitBinary(&jbv, DatumGetJsonb(value));
	}
...
{
	JsonPathExecResult res = executeJsonPath(jp, vars, DatumGetJsonb(jb), NULL,
											 NULL, NULL);
			   ...
			   bool *empty, List *vars)
{
	...
	bool		wrap;
	JsonValueList found = { 0 };
	JsonPathExecResult jper = executeJsonPath(jp, vars, DatumGetJsonb(jb),
											  &found, NULL, NULL);
	int			count;
	...
	if (wrap)
		return JsonbGetDatum(JsonbValueToJsonb(wrapItemsInArray(&found)));
	...
	if (first)
		return JsonbGetDatum(JsonbValueToJsonb(first));
					   ...
					   List *args, MemoryContext mcxt)
{
	...
	scan->path = DatumGetJsonPath(node->path->constvalue);
	scan->args = args;
...
JsonTableResetContextItem(JsonTableScanState *scan, Datum item)
{
	...
	res = executeJsonPath(scan->path, scan->args, DatumGetJsonb(item),
...
JsonTableNextJoinRow(JsonTableJoinState *state)
{
	...
	while (!state->u.join.advanceRight)
	{
		...
		if (state->u.join.cross)
		{
			...
			state->u.join.advanceRight = true;
		}
		else if (!left)
		{
			if (!JsonTableNextJoinRow(state->u.join.right))
...
JsonTableNextRow(JsonTableScanState *scan)
{
	...
	for (;;)
	{
		...
		oldcxt = MemoryContextSwitchTo(scan->mcxt);
		scan->current = JsonbGetDatum(JsonbValueToJsonb(jbv));
		MemoryContextSwitchTo(oldcxt);
