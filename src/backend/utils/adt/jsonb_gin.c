...
static Datum *
gin_extract_jsonpath_query(JsonPath *jp, bool pathOps,
						   int32 *nentries, Pointer **extra_data)
{
	...
	node = gin_extract_jsonpath_expr_recursive(&cxt, &root, false, path);
...
gin_extract_jsonb_query(PG_FUNCTION_ARGS)
{
	...
	else if (strategy == JsonbJsonpathStrategyNumber)
	{
		JsonPath   *jp = PG_GETARG_JSONPATH(0);
		Pointer	  **extra_data = (Pointer **) PG_GETARG_POINTER(4);

		entries = gin_extract_jsonpath_query(jp, false, nentries, extra_data);
...
gin_extract_jsonb_query_path(PG_FUNCTION_ARGS)
{
	...
	else if (strategy == JsonbJsonpathStrategyNumber)
	{
		...
		entries = gin_extract_jsonpath_query(jp, true, nentries, extra_data);
